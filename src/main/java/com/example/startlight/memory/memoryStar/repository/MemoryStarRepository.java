...
public interface MemoryStarRepository extends JpaRepository<MemoryStar, Long> {
    ...
    @Query("SELECT m.memory_id FROM MemoryStar m WHERE m.pet.pet_id = :petId AND m.star_node_id = :starNodeId ")
    Long findByPetIdAndStarNodeId(@Param("petId") Long petId, @Param("starNodeId") Long starNodeId);

    @Query("SELECT count(m) FROM MemoryStar m WHERE m.pet.pet_id = :petId")
    Integer countMemoryStarByPetId(@Param("petId") Long petId);

    @Query("SELECT m FROM MemoryStar m WHERE m.pet.pet_id = :petId")
    ...
    @Query("""
        ...
        join m.pet p
        where m.shared = true
          and (:category is null or p.animal_category = :category)
        order by m.updatedAt desc
    """)
    Page<MemoryStar> findByIsPublicTrueOrderByCreatedAtDesc(
            @Param("category") AnimalCategory category,
            Pageable pageable
    );
}
